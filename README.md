...
個人版桌面 Tube（三列）恢復不同列速（0.65×／1.10×／1.55×）與交錯起始排列；載入及調整視窗時，依卡片的透視投影範圍計算尺寸與列距，將前方各列的最小間距縮至 6px，同時避免上下裁切。後方卡片仍會有自然遮擋。窄於 760px 的 Tube 保留原有尺寸與密集排列；showcase 副本不受影響。
