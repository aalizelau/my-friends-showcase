...
桌面 Tube（三列）恢復不同列速（0.65×／1.10×／1.55×）與交錯起始排列；載入及調整視窗時，依卡片的透視投影範圍計算尺寸與列距，將前方各列的最小間距縮至 6px，同時避免上下裁切。後方卡片仍會有自然遮擋。窄於 760px 的 Tube 保留原有尺寸與密集排列。這些介面更新已同步至個人版與 showcase；兩份專案仍各自獨立。

桌面展示區維持原有高度，直接放大三列朋友卡片（一般桌面約 10–15%，窄視窗以不裁切為優先）；依卡片實際重疊的水平範圍計算列距，保留小間距及不同列速，手機不變。

滑鼠滾輪／觸控板在 Tube 或 Ring 區域內會轉動朋友，不同時捲動整個頁面；區域外照常捲頁。Tube 暫停自動旋轉時仍可手動捲動。雙指縮放、手機垂直觸控捲頁及長文字泡泡的內部捲動維持原有行為。
